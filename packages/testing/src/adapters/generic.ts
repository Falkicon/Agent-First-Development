  ...
  return {
    ...
    errors: {
      ...
      isRetryable: (code: string) =>
        ['TIMEOUT', 'NETWORK_ERROR', 'RATE_LIMITED'].includes(code),
    },
 ...
 */
async function genericFixtureValidator(
  fixture: unknown
  ...
  if (obj.data !== undefined) {
    if (!Array.isArray(obj.data)) {
      ...
      for (let i = 0; i < obj.data.length; i++) {
        ...
        if (!isDataItem(item)) {
          errors.push(
            `data[${i}] must have a 'command' field`
          );
        }
      }
    }
  ...
  if (obj.setup !== undefined) {
    if (!Array.isArray(obj.setup)) {
      ...
      for (let i = 0; i < obj.setup.length; i++) {
        ...
        if (!isDataItem(step)) {
          errors.push(
            `setup[${i}] must have a 'command' field`
          );
        }
      }
    }
